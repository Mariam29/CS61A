...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls >= 1:
        count = dice()
        if count == 1:
            #the first time i write return 1,but in fact i ignore one thing. 
            #if num_rolls > 1 the second test will not be happen,because return statement
            has_one = True 
        sum += count
        num_rolls -= 1
    if has_one:
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    s_dice = select_dice(score0,score1)
    is_prime(n)
    take_turn(,s_dice)
