...
def find_closest(location, centroids):
    ...
    """
    "*** YOUR CODE HERE ***"
    set = [distance(location,i) for i in centroids ]
    count = 0
    for i in set:
        if i == min(set):
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        pairs = pairs + [[centroid,restaurant]]
...
def find_centroid(restaurants):
    ...
    longitude = []
    for restaurant in restaurants:
        latitude=latitude+[restaurant_location(restaurant)[0]]
        longitude=longitude+[restaurant_location(restaurant)[1]]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    i = 0
    for xi in xs:
        Sxx += pow((xi-mean(xs)),2)
    for yi in ys:
        Syy += pow((yi-mean(ys)),2)
    for yi in ys:
        Sxy += (xs[i]-mean(xs))*(yi-mean(ys))
        i += 1
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for feature_fn in feature_fns:
        f = [find_predictor(user, reviewed, feature_fn)[0]]+[find_predictor(user, reviewed, feature_fn)[1]]
        f1 = f1 + [f]
    for feature_fn in feature_fns:
        g = g + [find_predictor(user, reviewed, feature_fn)[1]]
...
def rate_all(user, restaurants, feature_functions):
    ...
    "*** YOUR CODE HERE ***"
    d = {}
    dict1 = user_reviewed_restaurants(user, restaurants)
    for restaurant in RESTAURANTS:
        rating  = predictor(restaurant)
        if restaurant['name'] == dict1['name']:
            rating = user_rating(user, restaurant['name'])
        c = {restaurant:rating}
        d.update(c) 
    return d
